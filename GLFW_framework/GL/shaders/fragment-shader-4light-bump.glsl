...
void main() {
	vec3 normalMap = texture( bumpTex, fragTexCoord).rgb;
	vec3 normal = normalize(transpose(inverse(mat3(model))) * fragNormal);
	vec3 tangent = normalize(transpose(inverse(mat3(model))) * fragTangent);
	vec3 binormal = cross(normal,tangent);
	normal = normal*normalMap.b + tangent*2.0f*(normalMap.r-0.5) + binormal*2.0f*(normalMap.g-0.5f);
